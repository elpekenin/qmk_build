...
static bool render_scrolling_text_state(scrolling_text_state_t *state) {
    qp_dprintf("render_scrolling_text_state: entry (char #%d)\n", (int)state->char_number);

    // prepare string slice
    char *slice = malloc(state->n_chars + 1); // +1 for null terminator
    if (slice == NULL) {
        qp_dprintf("render_scrolling_text_state: could not allocate\n");
        return false;
    }
    ...
    qp_dprintf("render_scrolling_text_state: %s\n", ret ? "ok" : "fail");
    return ret;
}
...
deferred_token draw_scrolling_text_recolor(painter_device_t device, uint16_t x, uint16_t y, painter_font_handle_t font, const char *str, uint8_t n_chars, uint32_t delay, uint8_t hue_fg, uint8_t sat_fg, uint8_t val_fg, uint8_t hue_bg, uint8_t sat_bg, uint8_t val_bg) {
    qp_dprintf("draw_scrolling_text_recolor: entry\n");
    ...
    if (!scrolling_state) {
        qp_dprintf("draw_scrolling_text_recolor: fail (could not find free scrolling slot)\n");
    ...
    if (scrolling_state->str == NULL) {
        qp_dprintf("draw_scrolling_text_recolor: fail (could not allocate)\n");
        return false;
    }
    ...
    if (!render_scrolling_text_state(scrolling_state)) {
        ...
        qp_dprintf("draw_scrolling_text_recolor: fail (could not render first string)\n");
        return INVALID_DEFERRED_TOKEN;
    }
    ...
    if (scrolling_state->defer_token == INVALID_DEFERRED_TOKEN) {
        ...
        qp_dprintf("draw_scrolling_text_recolor: fail (could not set up scrolling executor)\n");
        return INVALID_DEFERRED_TOKEN;
    }

    qp_dprintf("draw_scrolling_text_recolor: ok (deferred token = %d)\n", (int)scrolling_state->defer_token);
...
void extend_scrolling_text(deferred_token scrolling_token, const char *str) {
    for (int i = 0; i < QUANTUM_PAINTER_CONCURRENT_SCROLLING_TEXTS; ++i) {
        if (scrolling_text_states[i].defer_token == scrolling_token) {
            ...
            if (new_pos == NULL) {
                qp_dprintf("extend_scrolling_text: fail (could not realloc)\n");
                return;
            }
